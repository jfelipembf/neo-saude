-- ═════════════════════════════════════════════════════════════════════════════
-- NEO SAÚDE — VENDA À VISTA (DINHEIRO/PIX) JÁ NASCE QUITADA
--
-- checkout_sale() sempre gravava o recebível como 'pending', mesmo pra
-- dinheiro/pix — que na prática o paciente já pagou NA HORA, no balcão (é o
-- que diferencia o PDV de um orçamento: aqui o dinheiro já entrou, ali é uma
-- promessa de pagamento futuro). Resultado: a venda ficava "pendente" e nem
-- o Dashboard nem a aba Vendas do Financeiro contavam o valor (os dois leem
-- receivable.status = 'paid' + received_at — ver 20260722150000_dashboard_
-- aggregates.sql e salesService.ts).
--
-- Cartão (crédito/débito) continua 'pending': o repasse da adquirente é D+N
-- (private.card_installment_plan), o dinheiro só cai na conta depois — esse
-- comportamento está correto e não muda aqui.
-- ═════════════════════════════════════════════════════════════════════════════
...
begin
  ...
  if v_total > 0 then
    ...
    gen as (
      ...
      -- Fica 'pending' — o dinheiro só chega quando a adquirente repassar.
      ...
      -- taxa — e o dinheiro já entrou NA HORA, no balcão. Nasce QUITADO.
...
comment on function public.checkout_sale(uuid, date, numeric, jsonb, jsonb) is
  ...
  'cartão nasce ''pending'' até a adquirente repassar. p_items = '
  '[{service_id, quantity}]; p_plan = [{method, amount, installments?, '
  'acquirer_id?, card_brand?, authorization_code?}] — bandeira/autorização '
  '(crédito/débito) vão pro notes do recebível. SECURITY DEFINER: exige '
  'patients/schedule, não finance — quem vende pelo perfil do paciente não '
  'tem acesso ao Financeiro (ver record_treatment_session).';
