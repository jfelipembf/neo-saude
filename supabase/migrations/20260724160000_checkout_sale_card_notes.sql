-- ═════════════════════════════════════════════════════════════════════════════
-- NEO SAÚDE — BANDEIRA/CÓDIGO DE AUTORIZAÇÃO NO PDV
--
-- O formulário de pagamento do PDV (PaymentForm.tsx) passou a pedir bandeira
-- (obrigatória) e código de autorização (opcional) em crédito/débito — mesmos
-- campos que PaymentModal.tsx já usa em Contas a Receber, mas lá nunca chegam
-- a ser persistidos (payment_entry.card_brand/authorization_code existem no
-- schema e nada grava neles). Aqui não repete a lacuna: os dois valores vão
-- pro `receivable.notes` do título gerado — texto livre já existente, sem
...
comment on function public.checkout_sale(uuid, date, numeric, jsonb, jsonb) is
  ...
  'installments?, acquirer_id?, card_brand?, authorization_code?}] — bandeira/'
  'autorização (crédito/débito) vão pro notes do recebível. SECURITY DEFINER: '
  'exige patients/schedule, não finance — quem vende pelo perfil do paciente '
  'não tem acesso ao Financeiro (ver record_treatment_session).';
