-- ═════════════════════════════════════════════════════════════════════════════
-- NEO SAÚDE — REVERTE: CARTÃO VOLTA A NASCER 'pending' (repasse da adquirente)
--
-- 20260724231500_checkout_sale_card_paid_now.sql marcava TODO pagamento do PDV
-- como 'paid' na hora, inclusive cartão — sem saber que o projeto já tem uma
-- arquitetura pronta e testada pra exatamente essa distinção:
--
--   · receivable.debtor (20260722233000_session_billing_link) — coluna GERADA:
--     acquirer_id preenchido ⇒ debtor='acquirer' — quem deve o repasse é a
--     ADQUIRENTE, não o paciente. O paciente já não deve nada (a maquininha
--     garantiu a venda na autorização); só a CLÍNICA ainda não recebeu o
--     dinheiro em conta.
--   · private.settle_card_receivables + cron 'neo-saude-finance-daily'
--     (20260722234000/20260723003000) — baixa AUTOMÁTICA (status→paid,
--     received_at=due_date, received_amount líquido) no dia do repasse, sem
--     ninguém clicar em nada. Isso É o "dá baixa sozinho quando chegar o dia".
--   · receivable_acquirer_never_overdue_ck — cartão nunca vira 'overdue'/
--     inadimplência, mesmo pendente.
--   · PaymentsTable.tsx (aba Pagamentos do perfil) já sabe ler tudo isso: some
--     o botão "Receber", mostra "Repasse em {data}" e explica que a venda já
--     foi garantida — SEM entrar na dívida que o paciente "deve hoje".
--
-- Ou seja: card pago-na-hora não só era desnecessário, quebrava esse sistema
-- inteiro (o item ficava com jeito de "já recebido pela clínica" quando na
-- verdade só o repasse ainda não caiu). Dinheiro/pix CONTINUAM 'paid' na hora
-- (não têm adquirente, não há repasse a esperar — debtor='payer' e quitado é
-- o estado real).
-- ═════════════════════════════════════════════════════════════════════════════
...
begin
  ...
  if v_total > 0 then
    ...
    gen as (
      ...
      union all
      -- Demais formas (dinheiro/pix, ou débito sem adquirente): à vista, sem
      -- taxa — o dinheiro já entrou NA HORA, no balcão. Nasce QUITADO
      -- (debtor='payer', mas já sem saldo aberto).
    ...
    select v_clinic,
           ...
           'Vendas', g.due_date, g.method, g.gross, g.fee,
           case when g.paid_now then 'paid' else 'pending' end,
...
comment on function public.checkout_sale(uuid, date, numeric, jsonb, jsonb) is
  ...
  'resto). Dinheiro/pix nascem status=''paid'' (dinheiro já entrou no balcão, '
  'debtor=''payer'' quitado); cartão nasce ''pending''/debtor=''acquirer'' — '
  'venda já garantida pela maquininha (paciente não deve mais nada), só falta o '
  'repasse cair, e private.settle_card_receivables (cron diário) baixa sozinho '
  'quando due_date chegar. p_items = [{service_id, quantity}]; p_plan = '
  '[{method, amount, installments?, acquirer_id?, card_brand?, '
  'authorization_code?}] — bandeira/autorização (crédito/débito) vão pro notes '
  'do recebível. SECURITY DEFINER: exige patients/schedule, não finance — quem '
  'vende pelo perfil do paciente não tem acesso ao Financeiro.';
