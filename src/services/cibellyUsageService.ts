...
import { calcularCustoUsd } from '@/lib/cibelly/pricing'
...
export async function listCibellyUsage(limite = 20): Promise<CibellyUsageRow[]> {
  const { data, error } = await supabase
    .from('cibelly_usage')
    .select('id, patient_id, provider, model, cost_usd, started_at, ended_at, text_input_tokens, audio_input_tokens, text_cached_tokens, audio_cached_tokens, text_output_tokens, audio_output_tokens')
