...
import type {
  ClinicSpecialty, PatientDocument, PrescribedMedication, Prescription,
} from '@/types/domain'
...
type Tipo = 'receita' | 'atestado' | 'exame' | 'relatorio'
...
/** Como um documento gravado é reconhecido depois: é pedido de exame ou não. */
const TITULO_EXAME = 'Solicitação de exame'
function ehPedidoDeExame(d: Prescription) {
  return d.title === TITULO_EXAME
}
...
export function ConsultationDocuments({
  ...
  const [tipo, setTipo] = useState<Tipo>(foco === 'exames' ? 'exame' : 'receita')
  ...
  const daLista = documentos.filter(d => (foco === 'exames' ? ehPedidoDeExame(d) : !ehPedidoDeExame(d)))
  ...
  function corpoDoDocumento() {
    ...
    switch (tipo) {
      case 'receita':
        return {
          titulo: 'Receituário', tipo: 'prescription' as const,
          html: prescriptionBody({ ...base, medications: medicamentos, text: texto }),
        }
      case 'atestado': {
      ...
      case 'exame':
        return {
          titulo: 'Solicitação de exame', tipo: 'document' as const,
      ...
      case 'relatorio':
        return {
          ...
          html: certificateBody({ ...base, text: texto || attendanceCertificateText(patientName, hojeBr) }),
        }
    }
  }
  ...
  function reimprimir(d: Prescription) {
    ...
    const html = d.type === 'prescription'
      ? prescriptionBody({ ...base, medications: d.medications, text: d.text })
      : certificateBody({ ...base, text: d.text ?? d.title })
  ...
  function emitir() {
    ...
    criar({
      ...
      title: doc.titulo,
      date: hojeBr,
      medications: tipo === 'receita' && medicamentos.length ? medicamentos : undefined,
      text: tipo === 'exame'
        ? [exames.join('; '), texto].filter(Boolean).join(' — ')
        : (texto || doc.titulo),
    }, {
      onSuccess: () => {
    ...
    <>
      ...
      >
        <div className={styles.formDocumento}>
          ...
          {tipo === 'receita' && (
            <>
              <MedicationsField value={medicamentos} onChange={setMedicamentos} />
              <Textarea
